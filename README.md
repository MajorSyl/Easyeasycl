...
- **Agent verification (`agent_verification`) is contact-us, not self-checkout.** The Plans & Pricing screen (`app/packages.tsx`) and Profile's "Become a Verified Agent" button no longer link to `/pay` for this product — they call `requestAgentVerification()` (`lib/contact.ts`), which opens a pre-filled `mailto:agents@easyfen.com`. An admin reviews the request manually, outside the automated payment flow; if approved, the admin contacts the agent separately to collect the fee and then grants verification through the same `admin_review_payment` path described below (`app/pay.tsx`'s submission screen still exists and still works for this — it's just no longer linked to from normal navigation). The pricing screen shows "Contact for pricing" for this product instead of an amount, and it's excluded from launch mode's free-claim flow entirely, regardless of `launch_mode_active`.
